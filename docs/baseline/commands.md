...
- `/code-review [low|medium|high|xhigh|max] [--comment] [target]` — bundled Skill; review the current diff for correctness bugs at chosen effort level; lower levels return fewer, higher-confidence findings while `high` through `max` give broader coverage and may include uncertain findings; pass `--comment` to post findings as inline comments on the current GitHub PR; pass a path or PR reference to review a specific target; formerly `/simplify`
...
- `/goal [condition|clear]` — set a goal; Claude keeps working across turns until the condition is met; with no argument shows the current or most recently achieved goal; use `clear`, `stop`, `off`, `reset`, `none`, or `cancel` to remove an active goal early
...
- `/run` — bundled Skill; launch and drive this project's app to see a change working; see [Run and verify your app](/en/skills#run-and-verify-your-app)
- `/run-skill-generator` — bundled Skill; teach `/run` and `/verify` how to build, launch, and drive your project's app from a clean environment by writing a per-project skill
...
- `/scroll-speed` — adjust mouse wheel scroll speed interactively with a ruler; available in fullscreen rendering only
...
- For deep code review prefer `/ultrareview` over `/code-review` when willing to trade time and tokens for thoroughness (formerly noted `/simplify`)
