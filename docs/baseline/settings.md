...
- Skills: `skillOverrides` (v2.1.129+, values `on`/`name-only`/`user-invocable-only`/`off`), `disableSkillShellExecution`, `maxSkillDescriptionChars` (v2.1.105+, per-skill character cap, default 1536), `skillListingBudgetFraction` (v2.1.105+, skill listing context budget, default 0.01)
