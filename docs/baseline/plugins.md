...
- Manifest fields: `name` (unique identifier and skill namespace), `description`, `version` (optional — falls back to git SHA), `author` (optional), `defaultEnabled` (optional boolean; default true)
...
- Plugins are discovered through marketplaces (registered separately) or direct local/url loading via `--plugin-dir <path>` or `--plugin-url <archive-url>`; plugins in `.claude/skills/` are auto-loaded without marketplace requirement
- The `/plugin` slash command manages installation, enabling/disabling, listing, and updates; `claude plugin init <name>` scaffolds a new plugin in `~/.claude/skills/`
...
- Bundled-file env vars: `${CLAUDE_PLUGIN_ROOT}` (plugin install dir), `${CLAUDE_PLUGIN_DATA}` (persistent data dir, survives plugin updates); use `${CLAUDE_CODE_PLUGIN_PREFER_HTTPS}` environment variable to direct plugin marketplace cloning to use HTTPS instead of SSH
...
- `extraKnownMarketplaces` settings key adds marketplace sources; sources are `github` (`{source: "github", repo: "owner/repo", skipLfs: true}` optional), `git` (`{source: "git", url: "..."}`), `directory` (`{source: "directory", path: "/local/path"}` for development), `hostPattern` (`{source: "hostPattern", hostPattern: "regex"}`), or `settings` (`{source: "settings", name: "...", plugins: [...]}`)
- Plugin dependencies system: plugins can declare dependencies in manifest via `dependencies` field; `claude plugin disable` refuses to disable if another plugin depends on it; `claude plugin enable` force-enables transitive dependencies
- Managed-only plugin policies: `strictKnownMarketplaces` (allowlist of marketplace sources, enforced before download), `blockedMarketplaces` (blocklist, enforced before download), `allowedChannelPlugins` (allowlist of channel plugins that may push messages, requires `channelsEnabled: true`), `pluginTrustMessage` (custom trust-warning text), `pluginSuggestionMarketplaces` (allowlist of org marketplaces whose plugins can be suggested)
...
- `/reload-skills` command re-scans skill directories without restarting

## Recommended
- Start with standalone (`.claude/`) for quick iteration; convert to a plugin only when ready to share; or use `claude plugin init <name>` to scaffold a plugin in `~/.claude/skills/` for automatic loading and team reuse
...
- Do not modify `.claude-plugin/plugin.json` mid-session expecting changes to apply automatically — run `/reload-plugins` after edits; for skill changes, also run `/reload-skills` if skills were modified
...
- Do not set `defaultEnabled: false` expecting the plugin to silently stay disabled — plugins with `defaultEnabled: false` must be explicitly enabled via `/plugin enable` or `claude plugin enable`
