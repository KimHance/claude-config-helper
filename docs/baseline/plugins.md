...
Plugin manifest schema full field list: `name`, `version`, `description`, `author` (object: `name`/`email`/`url`), `homepage`, `repository`, `license`, `keywords` (array), `$schema`, `skills` (component path override), `commands` (override), `agents` (override), `hooks` (override), `mcpServers` (override), `outputStyles` (override), `lspServers` (override), `experimental.themes`, `experimental.monitors`, `userConfig`, `channels`, `dependencies` (string or `{name, version}` entries).
...
Plugin agent supported frontmatter fields: `name`, `description`, `model`, `effort`, `maxTurns`, `tools`, `disallowedTools`, `skills`, `memory`, `background`, `isolation`; the only valid `isolation` value is `"worktree"`; `hooks`, `mcpServers`, and `permissionMode` are silently ignored for security reasons.
...
Plugin-specific MCP server paths: `${CLAUDE_PLUGIN_ROOT}/` prefix for scripts, MCP binaries, and config files bundled with plugin; `${CLAUDE_PLUGIN_DATA}/` for persistent state across plugin updates.
UserConfig field in manifest lets plugins prompt users for configuration values at enable time (API endpoints, tokens, etc.); supports `string`, `number`, `boolean`, `directory`, `file` types; sensitive values stored in system keychain.
Channels field in manifest allows plugins to inject content into conversation via MCP servers (for integrations like Telegram, Slack, Discord).
Plugin dependency versioning via semver constraints in `dependencies` array; supports cross-marketplace dependencies when allowlisted in root marketplace; auto-installs resolved dependencies; version conflicts between plugins can block installation.
Plugin output styles: plugins can ship custom output style definitions via `output-styles/` directory.
...
Declare plugin dependencies with version constraints when the plugin requires other plugins to work.
Tag plugin releases using the `claude plugin tag` command to enable version constraint resolution.
Use `userConfig` in the manifest to prompt users for configuration (API endpoints, tokens) rather than requiring manual settings edits.
Protect sensitive user configuration values by setting the `sensitive` flag, which stores them in the system keychain.

## Anti-patterns
Do not put `commands/`, `agents/`, `skills/`, `hooks/`, `.mcp.json`, etc. inside `.claude-plugin/` — only `plugin.json` belongs there.
Do not assume plugin agent frontmatter `hooks` / `mcpServers` / `permissionMode` are honored — they are silently ignored for security reasons.
...
Do not attempt to reference files outside the plugin directory via relative paths like `../shared-utils` — installed plugins cannot access files outside their directory due to caching. Use symlinks if external file access is needed.
Do not use absolute paths in plugin manifests or hook commands — all paths must be relative to plugin root and start with `./`.
Do not declare user configuration values that accept sensitive data without setting the `sensitive: true` flag — sensitive values are stored in the system keychain rather than plaintext settings.
Do not declare plugin dependencies without version constraints if the plugin relies on specific features — constrain the dependency version to ensure compatibility.
