...
- The `--plugin-dir` flag accepts both directories and `.zip` plugin archives
...
- Plugin themes are experimental; live in `themes/` as JSON with `name`, `base` (preset), `overrides` (sparse color tokens); declared under `"experimental": { "themes": [...] }` in `plugin.json`
- Plugin monitors are experimental; declared under `"experimental": { "monitors": [...] }` in `plugin.json`
