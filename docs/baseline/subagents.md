...
Subagents now receive the active effort level via `effort.level` JSON input field and `$CLAUDE_EFFORT` environment variable (v2.1.129+)
Skills can now reference the current effort level with `${CLAUDE_EFFORT}` in their content (v2.1.119+)
