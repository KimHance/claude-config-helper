...
- Subagents can now spawn nested subagents up to 5 levels deep as of v2.1.172; only the top-level subagent's summary returns to main conversation
- Fork mode is enabled by default as of v2.1.161 (configurable via `CLAUDE_CODE_FORK_SUBAGENT` env var, requires v2.1.117+): spawns a fork that inherits full conversation history, system prompt, tools, model; `/fork <directive>` triggers it; forks cannot spawn further forks
...
- Subagents inherit the main conversation's extended thinking configuration as of v2.1.198 (no per-subagent thinking setting)
- `permissionMode` values: `default`, `acceptEdits`, `auto`, `dontAsk`, `bypassPermissions`, `plan`, `manual` (alias for `default` as of v2.1.200)
...
- `background: true` always runs the subagent as a background task; default true as of v2.1.198 (previously false)
- `color` accepts `red`/`blue`/`green`/`yellow`/`purple`/`orange`/`pink`/`cyan` for display in task list and transcript
- Built-in subagents: Explore (read-only, inherits main model as of v2.1.198, capped at Opus on Claude API), Plan (inherits model, read-only, used in plan mode), general-purpose (all tools, inherits model), plus helpers `statusline-setup` (Sonnet) and `claude-code-guide` (Haiku)
