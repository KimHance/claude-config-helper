...
- As of v2.1.172, subagents can spawn their own subagents up to 5 levels deep; this enables sophisticated multi-level workflows where intermediate results never reach the main conversation
...
- `model` accepts `sonnet`/`opus`/`haiku`/`fable`/full model id (e.g. `claude-opus-4-8`)/`inherit`; defaults to `inherit`
...
- Plugin subagents appear in `/agents` and are referenced as `<plugin-name>:<agent-name>`; if in a subfolder, referenced as `<plugin-name>:<subfolder>:<agent-name>` (v2.1.178+)
...
- As of v2.1.186, use `Agent(model:opus)` syntax in permissions rules to block specific model spawns
- This restriction applies only to agents running as main thread (`claude --agent`); subagents themselves can spawn other subagents up to depth 5 (v2.1.172+)
...
- Foreground subagents block main conversation; background subagents run concurrently and as of v2.1.186 surface permission prompts in main session instead of auto-denying
- `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1` disables all background task functionality; Ctrl+B backgrounds a running task
- Fork mode (v2.1.161+): `/fork <directive>` or `Agent(fork, ...)` spawns a subagent that inherits full conversation history; enabled by default in interactive sessions; set `CLAUDE_CODE_FORK_SUBAGENT` env var to control
- Resume an existing subagent via `SendMessage` tool with the agent ID; or use standard `/resume` for background sessions; requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` for full team features
...
- Nested subagents: as of v2.1.172, a subagent can spawn its own subagents; depth limit is 5 levels from main conversation; as of v2.1.187, resumed subagents restore their original spawn depth
...
## Anti-patterns
- As of v2.1.172, subagents CAN spawn other subagents up to 5 levels deep; use this for sophisticated multi-level workflows where intermediate results never reach main conversation — this is now a supported pattern, not an anti-pattern
